...
export default function ServicesSection() {
    ...
    const handleRailScroll = () => {
        ...
        const currentProgress = Math.min(1, Math.max(0, rail.scrollLeft / max));
        setProgress(currentProgress);
        setCanScrollPrev(rail.scrollLeft > 4);
        setCanScrollNext(rail.scrollLeft < max - 4);

        const center = rail.scrollLeft + rail.clientWidth / 2;
        const cards = Array.from(rail.querySelectorAll('.service-tile'));
        ...
        <section className="services" id="services" ref={sectionRef}>
            ...
            <div className="services-inner wrap">
                ...
                <div className="services-progress" aria-hidden>
                    <span className="services-progress-fill" style={{ transform: `scaleX(${progress})` }} />
                </div>

                {/* ── Horizontal cards rail ── */}
                <div
                    className={`services-rail-wrap${canScrollPrev ? ' services-rail-wrap--left' : ''}${canScrollNext ? ' services-rail-wrap--right' : ''}${hasInteracted ? ' services-rail-wrap--interacted' : ''}`}
                >
                    <button
                        type="button"
                        className="services-rail-nav services-rail-nav--prev"
                    ...
                    >
                    {SERVICES.map((s, i) => (
                        <div key={s.id} className={`service-tile-shell${i === activeIndex ? ' is-active' : ' is-dim'}`}>
                            <div
                                className={`service-tile reveal reveal-delay-${i + 1}${i === activeIndex ? ' is-active' : ' is-dim'}`}
                                data-active={i === activeIndex ? 'true' : 'false'}
                                style={{ '--tile-bg': s.bg, '--tile-image': `url(${s.image})` }}
                    ...
                    <button
                        type="button"
                        className="services-rail-nav services-rail-nav--next"
                        onClick={() => scrollRailBy(1)}
